"""Smoke-test a Sahara Windows one-folder bundle."""
...
from scripts.build_windows_bundle import DIST_ROOT, bundle_name  # noqa: E402
...
def smoke_bundle(bundle_path: Path, *, with_index: bool = False) -> None:
    executable = bundle_path / "sahara.exe"
    if not executable.is_file():
        raise SystemExit(f"Missing bundled executable: {executable}")

    base_env = os.environ.copy()
    with tempfile.TemporaryDirectory(prefix="sahara-windows-bundle-smoke-") as tmp:
        root = Path(tmp)
        home = root / "home"
        home.mkdir()
        env = base_env | {
        ...
        (content / "notes.txt").write_text(
            "Sahara Windows bundle smoke test document about lunar geology.\n",
            encoding="utf-8",
        )
