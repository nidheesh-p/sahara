"""Build Sahara's macOS Apple Silicon PyInstaller bundle."""
...
def build_bundle(
    *,
    ...
    if not skip_platform_check and not is_macos_arm64():
        raise SystemExit(
            "macOS Apple Silicon bundles must be built on macOS arm64. "
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-platform-check",
        action="store_true",
        help="Allow non-macOS-arm64 execution for spec smoke tests.",
    )
    parser.add_argument(
        "--print-name",
        action="store_true",
