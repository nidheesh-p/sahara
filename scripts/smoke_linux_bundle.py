"""Smoke-test a Sahara Linux one-folder bundle."""
...
from scripts.build_linux_bundle import DIST_ROOT, bundle_name  # noqa: E402
...
def smoke_bundle(bundle_path: Path, *, with_index: bool = False) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sahara-linux-bundle-smoke-") as tmp:
        root = Path(tmp)
        home = root / "home"
        home.mkdir()
        env = base_env | {
        ...
        (content / "notes.txt").write_text(
            "Sahara Linux bundle smoke test document about lunar geology.\n",
            encoding="utf-8",
        )
