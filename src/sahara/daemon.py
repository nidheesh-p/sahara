...
def _daemon_main(config_path: Optional[Path]) -> None:
    ...
    from sahara.file_watcher import SaharaEventHandler, start_watching
    from sahara.notifier import notify_sync_error, notify_sync_complete
...
def poll_restores(db: "StateDB", s3: "S3Client") -> None:  # type: ignore[name-defined]
    ...
    from sahara.notifier import notify_restore_complete
...
def poll_restore_expiries(db: "StateDB", within_hours: int = 48) -> None:  # type: ignore[name-defined]
    ...
    from sahara.notifier import notify_restore_expiring
